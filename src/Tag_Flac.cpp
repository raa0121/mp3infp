// Tag_Flac.cpp: CTag_Flac クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
...
#if _WIN64
 #if _DEBUG
 #pragma comment(lib,"libflac_static_x64_d.lib")
 #else
 #pragma comment(lib,"libflac_static_x64.lib")
 #endif
#else
 #if _DEBUG
...
BOOL CTag_Flac::AddComment(LPCTSTR name,LPCTSTR value)
{
	CString _name(name);
	_name.MakeUpper();
...
BOOL CTag_Flac::DelComment(LPCTSTR name,int index)
{
...
BOOL CTag_Flac::GetComment(LPCTSTR name,int index,CString &strValue)
{
	strValue = _T("");
	//nameのなかからdwIndexの値を取得
	std::pair<CommentMap::iterator,CommentMap::iterator> itp = m_comments.equal_range(CString(name));
...
void CTag_Flac::GetCommentNames(CStringArray &strArray)
{
	//nameリストを返す
...
DWORD CTag_Flac::Load(LPCTSTR szFileName)
{
	...
	flac_sync_state   oy; /* sync and verify incoming physical bitstream */
	flac_stream_state os; /* take physical pages, weld into a logical stream of packets */
	flac_page         og; /* one Flac bitstream page.  Vorbis packets are inside */
	flac_packet       op; /* one raw packet of data for decode */
	...
	/********** Decode setup ************/
	flac_sync_init(&oy); /* Now we can read pages */
	
	while(1)
	{ /* we repeat if the bitstream is chained */
		int i;
		/* submit a 4k block to libvorbis' Flac layer */
		buffer = flac_sync_buffer(&oy,4096);
		bytes=fread(buffer,1,4096,fp);
		flac_sync_wrote(&oy,bytes);

		/* Get the first page. */
		if(flac_sync_pageout(&oy,&og) != 1)
		{
			fclose(fp);
			dwWin32errorCode = -1;
			return dwWin32errorCode;
		...
		flac_stream_init(&os,flac_page_serialno(&og));

		/* extract the initial header from the first page and verify that the
		Flac bitstream is in fact Vorbis data */
		...
		if(flac_stream_pagein(&os,&og)<0)
		{ 
			/* error; stream version mismatch perhaps */
//			fprintf(stderr,"Error reading first page of Flac bitstream data.\n");
			dwWin32errorCode = -1;
			break;
		}

		if(flac_stream_packetout(&os,&op)!=1)
		{ 
...
//			fprintf(stderr,"This Flac bitstream does not contain Vorbis "
//			  "audio data.\n");
			dwWin32errorCode = -1;
			break;
		}
		...
		(Flac) bitstream decoder.  Get the comment and codebook headers and
		...
		while(i<2)
		{
			while(i<2)
			{
				int result=flac_sync_pageout(&oy,&og);
				if(result==0)break; /* Need more data */
				/* Don't complain about missing or corrupt data yet.  We'll
				catch it at the packet output phase */
				if(result==1)
				{
					flac_stream_pagein(&os,&og); /* we can ignore any errors here
								 as they'll also become apparent
								 at packetout */
					while(i<2)
					{
						result=flac_stream_packetout(&os,&op);
						if(result==0)
						{
							break;
						}
...
*/
/*		m_strAudioFormat.Format(_T("Flac Vorbis, %ldHz, %ldkbits/s%s, %ldch"),
								vi.rate,
...
//		fprintf(stderr,"Encoded by: %s\n\n",vc.vendor);
		...
		/* flac_page and flac_packet structs always point to storage in
		libvorbis.  They're never freed or manipulated directly */
		vorbis_comment_clear(&vc);
		vorbis_info_clear(&vi);  /* must be called last */
		flac_stream_clear(&os);
	...
	while(1)	//dummy
	{
		...
		ov_callbacks callbacks = {
			...
			(int (*)(void *, flac_int64_t, int))				_fseeki64,
			(int (*)(void *))								fclose,
			(long (*)(void *))								ftell
		};
		...
		{
//			printf("Could not open input as an FlacVorbis file.\n\n");
			break;
		}
...
static void _v_writestring(flacpack_buffer *o,char *s, int bytes)
{
	while(bytes--)
	{
		flacpack_write(o,*s++,8);
	}
}
static int _commentheader_out(vorbis_comment *vc, char *vendor, flac_packet *op)
{
	flacpack_buffer opb;

	flacpack_writeinit(&opb);

	/* preamble */  
	flacpack_write(&opb,0x03,8);
	_v_writestring(&opb,"vorbis", 6);

	/* vendor */
	flacpack_write(&opb,strlen(vendor),32);
	_v_writestring(&opb,vendor, strlen(vendor));

	/* comments */
	flacpack_write(&opb,vc->comments,32);
	if(vc->comments)
	{
		int i;
		for(i=0;i<vc->comments;i++)
		{
			...
			{
				flacpack_write(&opb,vc->comment_lengths[i],32);
				_v_writestring(&opb,vc->user_comments[i], 
						vc->comment_lengths[i]);
			}
			else
			{
				flacpack_write(&opb,0,32);
			}
		}
	}
	flacpack_write(&opb,1,1);

	op->packet = (unsigned char *)_flac_malloc(flacpack_bytes(&opb));
	memcpy(op->packet, opb.buffer, flacpack_bytes(&opb));

	op->bytes=flacpack_bytes(&opb);
	op->b_o_s=0;
	op->e_o_s=0;
	op->granulepos=0;
...
DWORD CTag_Flac::Save(LPCTSTR szFileName)
{
	DWORD	dwWin32errorCode = ERROR_SUCCESS;
	...
	flac_sync_state		_oy; /* sync and verify incoming physical bitstream */
	flac_stream_state	_os; /* take physical pages, weld into a logical stream of packets */
	...
	while(1)
	{
		...
		//[file -> flac_sync_state]
		char *buffer;
		int bytes;
		buffer = flac_sync_buffer(&_oy,4096);
		bytes = fread(buffer,1,4096,fp);
		flac_sync_wrote(&_oy,bytes);

		//[flac_sync_state -> flac_page]
		flac_page og;
		if(flac_sync_pageout(&_oy,&og) != 1)
		{
			flac_sync_clear(&_oy);
			fclose(fp);
			fclose(fp_out);
		...
		//シリアル番号の取得
		_serial = flac_page_serialno(&og);
		
		//init flac stream
		flac_stream_init(&_os,_serial);
		...
		//[flac page -> flac stream]
		if(flac_stream_pagein(&_os,&og) < 0)
		{
//			state->lasterror = _("Error reading first page of Flac bitstream.");
			dwWin32errorCode = GetLastError();
			goto loaderr;
		}

		//[flac stream -> header_main]
		flac_packet header_main;
		if(flac_stream_packetout(&_os,&header_main) != 1)
		{
//			state->lasterror = _("Error reading initial header packet.");
		...
		{
//			state->lasterror = _("Flac bitstream does not contain vorbis data.");
			dwWin32errorCode = GetLastError();
			goto loaderr;
		}
		...
		int i = 0;
		flac_packet header_comments;
		flac_packet header_codebooks;
		flac_packet *header = &header_comments;
		while(i<2)
		{
			while(i<2)
			{
				int result = flac_sync_pageout(&_oy,&og);
				if(result == 0)
				{
				...
				{
					flac_stream_pagein(&_os,&og);
					while(i<2)
					{
						result = flac_stream_packetout(&_os,header);
						if(result == 0)
						{
							break;
						}
...
//				state->lasterror = _("EOF before end of vorbis headers.");
	...
	while(1)
	{
		...
		//
		flac_stream_state streamout;
		flac_stream_init(&streamout,_serial);
...
//		vorbis_commentheader_out(&_vc,&header_comments);
		...
		flac_stream_packetin(&streamout,&header_main);
		flac_stream_packetin(&streamout,&header_comments);
		flac_stream_packetin(&streamout,&header_codebooks);
		
		while((result = flac_stream_flush(&streamout,&ogout)))
		{
		...
		while(!eosout)
		{
			while(!eosout)
			{
				result = flac_sync_pageout(&_oy, &ogin);
				if(result == 0)
				{
				...
				{
					flac_stream_pagein(&_os, &ogin);
					while(1)
					{
						result = flac_stream_packetout(&_os, &op);
						if(result == 0)
						{
							break;
						}
						...
						{
							flac_stream_packetin(&streamout,&op);
							while(!eosout)
							{
								int result = flac_stream_pageout(&streamout, &ogout);
								if(result==0)
								{
									break;
								}
		...
		while(!eosin) /* We reached eos, not eof */
		{
			...
			while(1)
			{
				result = flac_sync_pageout(&_oy, &ogout);
				if(result==0)
				{
					break;
				}
...
cleanup:
		flac_packet_clear(&header_comments);
		flac_stream_clear(&streamout);
		break;
	}
...
#if 0
CString CTag_Flac::Acp2Utf8(const char *str)
{
	WCHAR wc[1024];
	CHAR wc8[1024*3];
...
CString CTag_Flac::Utf82Acp(const char *str)
{
	WCHAR wc[1024+1];
	CHAR ansi[1024+1];
...
//size=バイト数(NULL文字を含む)
void CTag_Flac::Ucs22Utf8(const WCHAR *str,char *buf,int size)
{
	int index = 0;
	int writePtr = 0;
	while(str[index])
...
//size=文字数(NULL文字を含む)
void CTag_Flac::Utf82Ucs2(const char *str,WCHAR *wBuf,int size)
{
	int index = 0;
	int writePtr = 0;
